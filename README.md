...
> **v0.4.1** —— 两种用法:① Claude Code **skill**(`/mba <brand>`);② 独立 **MCP server**([`mba-mcp-server`](https://www.npmjs.com/package/mba-mcp-server),已上 npm,14 工具),可接进 Claude Desktop / Cursor,支持**品牌订阅 → 自动重审 → delta 报告 → webhook/email 通知**的演化闭环。见 [§5.1](#51-mcp-server--从任何-mcp-agent-调-mba) 与 [MCP 快速上手](docs/13-mcp-quickstart.md)。
