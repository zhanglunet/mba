...
依赖:仅标准库(urllib 调 Messages API)。需环境变量 ANTHROPIC_API_KEY;
可选 ANTHROPIC_BASE_URL(默认 https://api.anthropic.com)、MBA_CLASSIFY_MODEL
(默认 claude-haiku-4-5-20251001)。**无 key 时优雅跳过**(不写 adopt,退回候选流,退出 0)。
...
MODEL = os.environ.get("MBA_CLASSIFY_MODEL", "claude-haiku-4-5-20251001")
...
def call_claude(items, api_key):
    """items: [{i, brand, title, applicable_dims}] → 模型返回的建议列表(等长)。"""
    payload = {
        "model": MODEL,
        "max_tokens": 2000,
        "system": SYSTEM,
...
def classify(cands, api_key):
    ...
    for start in range(0, len(cands), BATCH):
        ...
        try:
            sugs = call_claude(items, api_key)
...
def main(argv):
    ...
    if not api_key:
        print("classify: 未设 ANTHROPIC_API_KEY —— 跳过自动分类(退回候选流,人工 triage)。")
        return 0
