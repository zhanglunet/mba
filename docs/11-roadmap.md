...
- **② `keyword`/`news` 触发器去留;④ webhook receiver 补 `POST /webhooks/watch-event` → 待用户确认**(② 改动已发布 npm 包的订阅 schema;④ 属净新增入站端点)。现状核实:`scheduler` 只执行 `cron` 触发;`webhook` 触发由 receiver 的 `POST /webhooks/trigger` backing;**唯 `keyword`/`news` 是被 schema 收下却永不执行的静默 no-op**(docs/12 §3.1 还为 keyword 写了整节)。推荐:② 砍掉 `keyword`/`news` 两个死枚举(让 `subscribe_brand` 明确报错而非静默吞掉),`cron`+`webhook` 保留;RSS 接入(P3-B-3)需出网,留作独立功能。
