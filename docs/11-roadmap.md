...
| MCP Server 形态 | ✅ 已发布 npm | packages/mcp-server/ · **16 工具**(含舆情 `get_watch_events`/`record_watch_event`)· **10 面板 / 43 评委** · webhook 接收端 · `resume_audit` 续跑 · **实时联网研究**(`MBA_WEB_SEARCH`) · 225 tests · `npx -y mba-mcp-server@latest` |
...
**决策项 · 决策记录(2026-07-16 清账)**:
- **① 触发建议精确率验收线 0.3 vs 0.15 → 决定:保持 0.3,不降到 0.15。** docs/16 §8.3 已选路线 (a) 收紧触发规则(P1≥3、加权阈值 5→6),回测把 2 个最差误报压掉、真阳性零丢失;把验收线从 0.3 降到 0.15 是"移动球门"而非改进。硬信号驱动的修正幅度天然偏小(评委克制,单镜头 ±1 为主),用更严的触发规则 + 保持 0.3 的诚实验收,比放水阈值更符合反噪声原则。每次重审后继续在 §8.3 续账。
- **③ PDF 管线复活 or 声明按需 → 决定:声明按需,不复活入库。** PDF 由 `scripts/print_report.sh`(E2-6 已跨平台,走 `$PLAYWRIGHT_BROWSERS_PATH/chromium`)按需生成,不进仓库;所有报告 `has_pdf: false` 是**有意状态**——报告以 HTML 为对外主格式(内联 Chart.js/Mermaid 可交互),PDF 非必需,按需可出。
- **⑤ zhanglan 留 seed → 决定:维持 seed。** 唯一干净一手仅澎湃深访 2114 字,凑不齐 SOP §3 的 ≥30 逐字/≥80% 一手门槛;守门槛不硬蒸(反捏造立身),等更多一手语料再升 full。consumer-cn 面板 4/5 full 已可用。
- **② `keyword`/`news` 触发器去留 → 决定:砍掉死枚举(用户确认「按推荐做」)。** `subscribe_brand` 的 trigger enum 从 `['cron','webhook','keyword','news']` 收到 `['cron','webhook']`(两者均已接线:`cron` 由 scheduler 轮询、`webhook` 由 receiver `POST /webhooks/trigger` 入站);`keyword`/`news` 此前被 schema 收下却永不执行(scheduler 只跑 `cron`),是"订阅了却永不触发"的静默 no-op,移除后明确报错而非静默吞掉。落点:`server.ts` enum、`types.ts` `Trigger` 联合类型、e2e 负测试(keyword/news 被拒 + cron/webhook 仍收,vitest 225 全绿)、docs/12 §3(标为未接线设计草案)。RSS 接入(P3-B-3)需出网,留作独立功能再把 type 引回。
- **④ webhook receiver 补 `POST /webhooks/watch-event` → 决定:暂缓(用户确认「按推荐做」)。** 现在 watch 事件靠人工采集 + 校验入库(反捏造可控);没有可信自动入站源时,加一个入站端点价值有限,且引入"谁能推、如何防伪造事件"的信任边界问题。等有真实入站信号源再建。receiver 现有 `POST /webhooks/trigger`(直接触发重审)不受影响。
...
**事项**:第三场创始人晚餐 —— 任正非(华为)× 马斯克(Tesla)「全栈自研的两种活法」
**触发**:全仓盘点下一步计划(用户四选之"加创始人晚餐");借特斯拉入库顺势铺
**完成内容**:
- `collabs/huawei--tesla.yaml`:5 镜头假想对谈。核心张力 = **全栈自研的双刃**——两人都信"什么都自己造"
  (任正非被制裁逼出"备胎/自立",马斯克被产能逼出"工厂即产品"),本可在部件上互补(华为鸿蒙座舱/ADS 智驾 +
  特斯拉整车),却也正因都执着掌控全栈,天然是**竞争而非合作**——尤其中国智驾市场 ADS vs FSD 是正面对手。
  身份镜头对照最鲜明:最高调的创始人图腾(马斯克/X)× 最低调的集体领导(任正非/轮值制/不上市)
- **诚实盒(反炒作)**给足:地缘/出口管制使美国 EV 龙头 × 受制裁中国科技公司的实质合作现实中几乎不被允许 =
  最大不搭;ADS 与 FSD 的结构性利益冲突;身份文化南辕北辙;交付/智驾/制程数字待核验——如实说明二者更像
  "对手"而非天然盟友
- **首页亮点不抢**:本场设 `featured: false`,首页「亮点」块仍为 deepseek--nvidia(芯片与模型);首页仅
  "共 2 场 → 共 3 场"计数更新。两位均为 MBA 评委(renzhengfei/musk),表达 DNA 充分,反捏造更稳
- 反捏造:发言为 AI 演绎双方公开立场(华为的冬天/备胎/灰度 · 第一性原理/工厂即产品),paraphrase 不冒充
  逐字原话;华为智能汽车解决方案(不造车、做增量部件/鸿蒙智行/ADS)与特斯拉 FSD 为公开已知业务,近期需 live 核验
- QA:`validate_collabs`(3 场合规 + 16 组 selftest)+ `check_consistency`(11 格,3 场晚餐 + 亮点对齐)+
  headless(新晚餐页/首页/组合器 0 JS 错误 · disclaimer + 诚实盒俱在)
**MBA 现覆盖**:23 品牌 + 23 位创始人 + 6 产业 + **3 场创始人晚餐**
**commit**:本 PR

### 2026-07-16(续十三)

**事项**:清账决策项 ②(砍 keyword/news 死枚举)+ ④(暂缓)落定
**触发**:用户「按推荐做」
**完成内容**:
- **② 砍死枚举**:`subscribe_brand` 的 trigger enum 从 `['cron','webhook','keyword','news']` 收到 `['cron','webhook']`。根因——`scheduler` 只执行 `cron`、`webhook` 由 receiver `POST /webhooks/trigger` backing,唯 `keyword`/`news` 被 schema 收下却永不执行(静默 no-op,"订阅了却永不触发")。改法:`server.ts` enum + `types.ts` `Trigger` 联合类型收窄 + **e2e 负测试**(keyword/news 被 `isError` 拒、cron/webhook 仍收,vitest 224→**225** 全绿)+ docs/12 §3 标为"未接线设计草案"、§3 顶部加移除说明、P3-B-3 行更新。typecheck/build 过。**不自动重发 npm**(bump/publish 属独立显式步骤)。RSS 接入(P3-B-3)需出网,留作独立功能再引回
- **④ 暂缓**:webhook receiver 不补 `POST /webhooks/watch-event`。理由:watch 事件现靠人工采集+校验入库(反捏造可控),无可信自动入站源时加端点价值有限,且引入"谁能推/如何防伪造事件"的信任边界;等真实入站源再建。现有 `POST /webhooks/trigger` 不受影响
- **①③⑤**(续:2026-07-16 已在决策记录落定):① 验收线保持 0.3;③ PDF 声明按需;⑤ zhanglan 维持 seed
**至此 G 系列 5 个「待产品拍板」决策项全部清账**(①③⑤ 记录 · ② 落地代码 · ④ 记录暂缓)
**commit**:本 PR

### 2026-07-16(续十四)

**事项**:8 个新品牌 watch 冷启动 —— 补舆情事件流 + 生成舆情驾驶舱
**触发**:用户「全站的舆情驾驶舱,还没有更新新加入的品牌」
**背景**:本轮新增的 8 家科技巨头(deepseek/nvidia/apple/google/microsoft/amazon/huawei/tesla)入库时只加了 watch/matrix 行,没有 `watch/<slug>/events.yaml`,所以 `build_watch_cockpit.py` 不给它们出驾驶舱(只对有事件的品牌生成)。
**完成内容**:
- **首批冷启动 27 条可溯源事件**(deepseek/nvidia/amazon 各 4,其余各 3)。**采源全部走公开 RSS**:官方 newsroom/blog(NVIDIA/Apple/Microsoft/Amazon/Google)+ **Google News RSS search**(deepseek/tesla/huawei + 各家监管/市场角度)。**url / quote / date 全部取自源 feed**(用一次性生成器实时抓取,零手造,杜绝伪造 URL/标题);`direction / dim / severity / lens_map` 为 model-judged 分类并如实标注。含正负面均衡(如 NVIDIA 对华销售停滞、Tesla 致命事故调查、Huawei Pura 评测失手、微软裁员、亚马逊 FTC 罚款 vs 各家正面产品/财务信号)
- **反捏造**:quote 为源站原标题(Google News 去尾部来源名),事件文件头注明"model-judged 分类、数字需 live 核验";未逐字核验正文,只用标题
- **驾驶舱生成**:8 张 `/watch/<slug>/cockpit.html`(零依赖静态,headless 0 JS 错误);首页每张新卡自动挂上「舆情驾驶舱 →」入口(`build_home_cards` 依 events 存在与否条件渲染)
- QA:`validate_watch`(23 事件文件 / 127 事件全合规)+ `check_consistency`(11 格,首页对齐)+ 8 驾驶舱 headless 0 JS 错误。矩阵维度守界(huawei 无 W5=未上市、各家 W3=非招投标 均未违规)
**MBA watch 覆盖**:15→**23 品牌全部有事件流 + 驾驶舱**
**commit**:本 PR

### 2026-07-17

**事项**:新增受监控品牌 Palantir + 创始人 Alex Karp(完整审计入库)· **第 4 个真实 --panel-drop**
**触发**:用户「增加公司:palantir」
**完成内容**:
- **Palantir 完整审计**(vc-en,**--panel-drop pthiel**):安德森 45 · 格雷厄姆 41 · 纳瓦尔 42 · 霍夫曼 37 = **165/200 · 8.25**(高分梯队,Google 8.44 之下、DeepSeek 8.00 之上)。**Leverage 8.75 领跑**——多年期政府合同 + 前置部署工程师 + 本体/平台的"无聊却不可迁移"绑定型护城河。**核心分歧在 Identity(Δ3,全卷最大)**:安德森(美国活力/国防科技)"敢选边是最强资产"10 vs 霍夫曼(建制/信任视角,总分 37 全卷最低)"极化是天花板"7。Signal(7.0)最弱:真实增长之上悬着估值溢价(90x P/E)的追问
- **第 4 个真实 --panel-drop**:彼得·蒂尔(pthiel)是 Palantir 联合创始人兼董事长,`self-conflict.yaml` 里 pthiel.orgs 已含 Palantir → 审计时 drop,4 评委打分、满分 200。`check_self_conflict --brand Palantir --panel vc-en` 正确命中(继微软/华为/特斯拉)
- **创始人 Alex Karp**:CEO / 联合创始人(哲学博士,师从哈贝马斯圈层),非 MBA 评委 → perspective_slug 留空(同 google/佩奇);事实锚 palantir.com/about(2003 founded)+ 公开记录,近期估值/AIP/标普500 需 live 核验。彼得·蒂尔为联合创始人兼董事长(审计已 drop)。产业归类:企业服务·安全
- **全套耦合**:report.md(10 段 + 数字自洽 /200)+ 手写 report.html(4 评委 + drop 卡,Leverage 最强紫、Identity Δ3 核心分歧橙、Signal 最弱)+ panel.yaml(drop: [pthiel])+ reports-meta + published-reports.txt + watch/matrix + founders/palantir.yaml + **watch 冷启动 4 事件(RSS 采源)+ 驾驶舱** + api 重生成 24 reports
- 反捏造:评委评分/对话 AI in-character 模拟(disclaimer),Palantir 事实取自官网 about 锚点,估值/合同/商业占比需 live 核验;watch 事件 url/quote/date 取自源 feed、分类为 model-judged
- QA:report/integrity(/200 自洽)/html 全过 · render-qa canvas=2 离线(bodyH=5913)· check_self_conflict(命中并 drop)+ check_consistency 11 格 · 首页 0 JS 错误(24 品牌)· palantir 驾驶舱 0 JS 错误 · founders/watch 各 24 对齐
**MBA 现覆盖**:**24 品牌** + 24 位创始人 + 6 产业 + 3 场创始人晚餐 · 全品牌舆情驾驶舱
**commit**:本 PR

### 2026-07-18(续二)

**事项**:watch 每日自动发现 workflow(用户问"能否自动/加密度")+ 首页近期异动块 + kimichat K3
**触发**:用户「K3 发布了 kimi 舆情没信号?」「watch 能否自动/按小时?」「首页设异动提醒(参考 oaf.world 今日异动)」
**判断(诚实版)**:watch 判断字段(dim/severity/direction/lens)是**刻意留人工/评委**的——反捏造立身之本(docs/15 §边界:不做舆情实时评分、direction 是显式标注)。**按小时不做**(舆情非小时级、RSS 多返重复、成本/噪音高)。选定**每日自动发现→PR 待审**:自动化"发现"、保留 curation 闸门。
**完成内容**:
- **kimichat 补 K3 事件**(PR #162):4 条可溯源 K3 发布事件(Reuters/Axios/Yahoo Finance/R&D World),信号链通到首页 + evaluate_triggers ⚡建议重审
- **首页「重大舆情·近期异动」块**(PR #162):`build_home_cards` 加 `render_alerts`,跨品牌汇总**未消费** P0/P1(确定性,--check 可守),置顶前 8;K3 一进首页即置顶
- **每日发现 workflow**(本 PR):`fetch_candidate.py` 加 `discover` 子命令(每品牌 Google News RSS → dedup 现有事件 → emit 候选草稿,**判断字段 TODO**、标题/日期/URL 取自源 feed;`--limit` 封顶防噪音灌水)+ `.github/workflows/watch-discover.yml` 每日 cron 跑它、写 `watch/_candidates/<date>.md`、**开 PR 待审**(不自动合并)。新 workflow 首启需 Actions 页 Approve(G-daily 先例)
**明确不做**:自动打分、按小时、自动合并候选、抓不可达反爬源(守 docs/15 §边界)
**commit**:PR #162(K3 + 首页异动)· 本 PR(discover + workflow)
