...
- **名额优先级(2026-07-27 修)**:自备 RSS 与知乎泛查询共用「社区 ≤1/4 名额」,
  但原实现把它排在知乎**后面** —— meituan 实测 `soc` 里知乎 53 条、自备 RSS 10 条,
  `soc[:3]` 全被知乎占满,**自备 RSS 被 100% 挤掉且哨兵不响**(源确实抓到了条目,
  只是没进候选,是最难发现的那种失效)。改为桶内 `own` 优先(**显式配置 > 顺带召回**),
  总名额不变。名额逻辑抽成纯函数 `_apply_quota()` 并补 6 组真跑断言(不是读源码)。
- **候选 md 的 `source_type` 不再写死 `media`**(同日修):JSON 侧一直按召回路径给对了,
  但给人看的候选 md 模板硬编码 `media` —— 官方公告与微博条目**人工照抄就是错标**。
  改为按召回路径填、注释改成"人工核对"。
- **密钥不进仓库**:本仓库公开,RSSHub 的 key 必须跟在 URL 上,故 meta 只存
  `?key=${RSSHUB_KEY}` 占位符,真值走 Actions secrets、由 `expand_secrets()` 在 runner 内存展开;
  变量缺失时**跳过该源并明确喊「环境变量未设置」**(不拿空 key 去请求 —— 那会 403,
  把配置错误伪装成路由故障);日志与候选 md 打印的永远是占位符模板串。
  门禁 `check_consistency` 第 16 格拦明文密钥。操作见 `docs/29`。
...
### ⏸ 接线状态:**暂缓**(用户决定,2026-07-27)

② 已实测打通,但**暂不接线** —— 用户要求"先让 MBA 完全线上运行"。
理由不是技术:这条线要求**一台 24×7 醒着的机器**,与"零本机依赖"直接冲突
(机器睡了/断网/搬家 → 当天该源静默消失)。代价与收益的对照表、以及"什么时候值得开"的
判断标准,见 **[`docs/30`](30-online-operation.md)** 与网页版 `mbabrand.com/runtime.html`。

**管道保持就位、零维护成本**:没配 `rss_feeds` 的品牌行为与改动前完全一致(已回归验证)。
想开时两步:① 建命名隧道 + `ACCESS_KEY` → GitHub secret `RSSHUB_KEY`(`docs/29`);
② meta 加一行 `rss_feeds`(密钥写 `${RSSHUB_KEY}` 占位符)。
