...
```
/mba TestBrand --dry-run
/mba TestBrand --dry-run --panel vc-en
```

v0.2.37 起已实现。Lead 在 Phase 0 完成路径解析 / 面板解析 / FRESH-EVOLUTION 检测后打印一份结构化计划，然后退出——不发出任何网络请求，不写任何文件。

输出格式示例：

```
### MBA Dry-Run Plan — TestBrand
Mode:          FRESH (no prior report)
Panel:         default (5 judges)
Judges:        jobs ✓ | fusheng ✓ | likejia ✓ | wu-jundong ✓ | zhang-yiming ✓
Wuying leg:    YES
Dimensions:    1 2 3 4 5 6 7 (all default)
Output path:   ~/mba/published/reports/testbrand/
Flags active:  --dry-run
— No files written. No network requests made. —
```

适合在正式跑之前确认面板 / 评委 / slug / 输出路径是否正确。
