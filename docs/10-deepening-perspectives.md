...
> `tomford`/`burton` 均判 deepen(其中 zhanglan≈78%、zhangbangxin≈82% 偏紧,取数时诚实标注一手占比)。
...
**seed → 待深化(10)**——行业 panel 评委(43 = 33 full + 10 seed;下表按面板列主要中文/待评估者,luxury-en 剩余 4 位另见 docs/11):

| 首次所属 panel | seed 评委(slug) | 取数难度 |
|---|---|---|
| ai-app-cn | `wanghuiwen` | `zhuxiaohu`+`yangzhilin` 已 full → **ai-app-cn 4/5**;`wanghuiwen` 侦察判 deepen(三篇 36kr 演讲全文) |
...
| consumer-cn | `zhongshanshan` `zhanglan` | 侦察均判 deepen:钟=2024 央视《对话》专访+2025 养生堂年会演讲(≈90%);张兰=澎湃深访+新浪睿见(≈78%,直播口语无实录须诚实标注) |
| cross-border | `chennian` `zhuangshuai` | 侦察均判 deepen:陈年=digitaling 口述+虎嗅 828 衬衫发布会演讲全文(≈90%);庄帅=新浪博客+腾讯云署名专栏 self-writing(≈92%) |

> **状态(2026-07-07 更新)——剩余 10 位;12 路侦察证明其中 9 位可 route-A deepen、仅 arnault marginal(见上侦察结论)。**
> 本会话已把 **18 位**做到 full:paulg/naval/pmarca/pthiel/rhoffman + cucinelli + 黄峥/俞敏洪/阳萌/罗永浩/
> 周受资/江南春/Sal Khan + **徐新 + 张磊 + 沈南鹏 + 朱啸虎(收官 vc-cn 5/5)+ 杨植麟(ai-app-cn→4/5;均 route-A 验证)**。
...
> - `yangzhilin`/`zhuxiaohu`/`chennian`/`zhuangshuai`——**未逐个 route-A 重扫,先别下"二手"结论**;
>   `zhongshanshan`(钟睒睒)极少公开发声;`zhanglan`(张兰)以直播/访谈为主。
