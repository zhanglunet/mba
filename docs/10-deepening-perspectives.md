...
vc-en(2026-07-07 深化):`paulg` —— **首个从 seed 升到 full 的行业 panel 评委**。
6 路 research 由 28 篇 paulgraham.com 一手 essay(2001–2024,全文抓取)蒸馏,28 条 verbatim
金句逐条比对源站校验。**一手占比(按引用计)= 93%(40/43:37 篇 paulgraham.com essay + 3 条
@paulg 自述 vs 2 维基 + 1 YC),满足 §3 的 ≥80% gate。** `check_structure` OK、`quality_check`
6/6(注:该脚本内建的"一手占比"字段只是 SKILL Sources 段的**标签词粗代理**,阈值 50%,不是 §3
的语料一手占比——它对 05/06 的 "Primary + Secondary" 标签会低估,读数 67%;§3 gate 以上面按引用计的 93% 为准)。

**seed → 待深化(27)**——2026-06 新建的行业 panel 评委:
...
| vc-en | `pmarca` `pthiel` `naval` `rhoffman`(`paulg` ✅ 已 full,2026-07-07) | 英文一手充足(essays/书/听证);放行后用 curl 走出口代理抓全文即可深化 |
...
> 建议优先级:**vc-en 剩余 4 位**(`pmarca` / `pthiel` / `naval` / `rhoffman`,英文一手最易
> 拿全文)→ `salkhan` / `shouzichew` / `huangzheng`(有英文 / PDF 一手)→ 其余中文人物(配 Wuying)。

> **状态更新(2026-07-07)——英文腿已通,中文腿仍挂起。**
> - ✅ **英文一手已可读**:CCR 环境出口网络策略把 `paulgraham.com` / `www.paulgraham.com` /
>   `www.ycombinator.com` / `en.wikipedia.org` 加进 allowlist 后,**`curl`(带浏览器 UA,走
>   `$HTTPS_PROXY` 出口代理 + `--cacert /root/.ccr/ca-bundle.crt`)可 200 抓到全文**。
>   `paulg` 即用此法深化到 full(见 §6 FULL 列)。
> - ⚠️ **注意**:`WebFetch` 工具本身对 paulgraham.com 仍 403 —— 它走 Anthropic 服务端出口(不
>   经本地代理),且 paulgraham.com 源站封非浏览器 UA。**放行后深化英文人物请用 `curl`,不要
>   依赖 WebFetch。** 验证放行是否生效:`curl -sS "$HTTPS_PROXY/__agentproxy/status"` 看
>   `recentRelayFailures`;或直接 `curl` 目标站看是否 200。
