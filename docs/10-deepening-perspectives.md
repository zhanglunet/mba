...
**FULL(15)**——default / auto / security-cn-global 三套早期 panel 的评委:
...
**seed → 待深化(28)**——2026-06 新建的行业 panel 评委:
...
| vc-en | `pmarca` `paulg` `pthiel` `naval` `rhoffman` | 英文一手充足(essays/书/听证),可 WebFetch 放行后深化 |
...
> 建议优先级:**vc-en 5 位**(英文一手最易拿全文)→ `salkhan` / `shouzichew` / `huangzheng`
> (有英文 / PDF 一手)→ 其余中文人物(配 Wuying)。

> **当前 blocker(as of 2026-06-18)**:Wuying tenant `1685405689137018` 仍在免费档,
> `GetLink` 报 400(Pro/Ultra 限定);沙箱内 `WebFetch` 对一手源站
> (paulgraham.com / 维基 / CF 保护的中文站)实测 403。两条腿都未通,深化工作挂起
> —— 任一解封即可启动,优先级仍按上一段。解封后请回填本行,删除"挂起"措辞。
