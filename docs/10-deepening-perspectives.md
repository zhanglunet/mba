...
> **状态(2026-07-07)——已触及"干净一手"的边界,剩余 15 位按 ≥80% 一手门槛暂留 seed。**
> 本轮(curl + Full 出口)已把**所有能拿到 curl-可达、逐字、≥80% 一手**的判官做到 full(本会话 12 位:
> paulg/naval/pmarca/pthiel/rhoffman + cucinelli + 黄峥/俞敏洪/阳萌/罗永浩/周受资/江南春/Sal Khan)。
> 剩余 15 位卡在**一手不可达或仅二手**:
> - `zhangbangxin`(张邦鑫)**已侦察**:芥末堆(2013 记者特写,"张邦鑫希望…"转述)、CEIBS 校友刊(记者稿)
>   均为**记者转述+零散引用**,非第一人称逐字实录;多知网 / jyb.cn(中国教育报)**403 挡**。其名句
>   (「教育的本质是爱而不是科技」「好的教育是传递价值观而非知识」「中医/西医教育模式」)全为**媒体转述**,
>   无法逐字校验 → **按 §3 反捏造 + ≥80% 门槛暂留 seed**(强做会破坏 verbatim 底线)。
> - `wanghuiwen`(王慧文)清华产品课**只有旁听者整理笔记**(非逐字),firewall 做不可靠 → seed。
> - `yangzhilin`/`zhuxiaohu`/`zhanglei`/`xuxin`/`shennanpeng`/`chennian`/`zhuangshuai`——多为**记者访谈=二手**;
>   `zhongshanshan`(钟睒睒)极少公开发声;`zhanglan`(张兰)以直播/访谈为主 → 均难达 80% 逐字一手。
> - `zhanglei`(张磊)《价值》(2020)、`张兰`等的书 = 版权内容,不可 curl 逐字引。
