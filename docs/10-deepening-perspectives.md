...
**seed → 待深化(19)**——行业 panel 评委(43 = 24 full + 19 seed;下表按面板列主要中文/待评估者,luxury-en 剩余 4 位另见 docs/11):

| 首次所属 panel | seed 评委(slug) | 取数难度 |
|---|---|---|
| ai-app-cn | `yangzhilin` `wanghuiwen` `zhuxiaohu` | 中文一手,建议 Wuying |
| edu-cn | `zhangbangxin` `salkhan` | 张中文(Wuying);Sal Khan 英文(可 WebFetch) |
| vc-cn | `zhanglei` `xuxin` `shennanpeng` | 中文一手,建议 Wuying |
| consumer-cn | `jiangnanchun` `zhongshanshan` `luoyonghao` `zhanglan` | 中文一手,建议 Wuying |
| cross-border | `shouzichew` `chennian` `zhuangshuai` | 周受资听证(英文 PDF,可 WebFetch);陈年/庄帅中文 |
