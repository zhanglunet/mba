...
| **评委市场 / 第三方上架** | 第一版只支持 5 个内置评委 + 用户本地添加自定义评委,不做评委发现/分发 |
...
5 套 perspective skill 是 MBA 的灵魂,一起包发布。允许用户**追加**自定义评委,但不允许**移除**内置 5 评委(可以选择不让某个评委参与某次审计,但 server 内的内置数据始终在)。
...
**移除自定义评委:** 删 `~/.mba/judges/<name>.md` 即可,server 启动时重新扫描。**内置 5 评委不可删**。
...
│       │   │   └── phase-5-merge.ts
│       │   ├── judges/
│       │   │   ├── built-in.ts          运行时把 5 套 perspective 内联进 bundle
